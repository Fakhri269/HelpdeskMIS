...
// Statuses a regular user is allowed to set on their own ticket.
// (Only reopening / confirming resolution — assignment & work-in-progress
// statuses stay controlled by the helpdesk agent.)
const USER_SETTABLE_STATUSES = ["Open", "Resolved", "Closed"]
...
export default function UserPortalPage() {
  ...
  // Ticket detail / status-change state
  ...
  const handleStatusChange = async (ticketId: string, newStatus: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatusError("Gagal mengubah status. Silakan coba lagi.")
      }
    } catch {
      setStatusError("Gagal mengubah status. Silakan coba lagi.")
    } finally {
                ...
                <div className="space-y-2">
                  {myTickets.map(ticket => {
                    ...
                    return (
                      <div
                        key={ticket.id}
                        onClick={() => { setStatusError(""); setSelectedTicket(ticket) }}
                        className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4 hover:shadow-md hover:border-blue-100 transition-all group cursor-pointer"
                      >
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              {unitKerjaList.length > 0 && (
                <div>
                  ...
                  <Select value={unitKerjaId} onValueChange={(value) => setUnitKerjaId(value ?? "")}>
      ...
      {selectedTicket && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <p className="text-xs text-slate-400 font-medium mb-0.5">{selectedTicket.ticketNumber}</p>
                <h2 className="text-lg font-bold text-slate-800 leading-tight">{selectedTicket.title}</h2>
              </div>
              <button
                onClick={() => setSelectedTicket(null)}
                className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 transition-colors flex-shrink-0 ml-3"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              {/* Current status */}
              <div>
                <Label className="text-sm font-semibold text-slate-700 mb-2 block">Status Saat Ini</Label>
                {(() => {
                  const cfg = STATUS_CONFIG[selectedTicket.status] || STATUS_CONFIG["Open"]
                  const Icon = cfg.icon
                  return (
                    <Badge className={`text-xs px-3 py-1 border ${cfg.className}`}>
              ...
              {selectedTicket.description && (
                <div>
                  <Label className="text-sm font-semibold text-slate-700 mb-1.5 block">Deskripsi</Label>
                  <p className="text-sm text-slate-600 whitespace-pre-line bg-slate-50 rounded-xl p-3 border border-slate-100">
                    {selectedTicket.description}
                  </p>
                </div>
              )}
              ...
              {/* Change status */}
              <div>
                <Label className="text-sm font-semibold text-slate-700 mb-2 block">Ubah Status</Label>
                  ...
                  {USER_SETTABLE_STATUSES.map(s => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border text-xs font-semibold transition-all disabled:cursor-not-allowed ${
                          active
                            ? "border-blue-500 bg-blue-50 text-blue-700 shadow-sm"
                ...
                <p className="text-xs text-slate-400 mt-2">
                  Status lain (Diproses, Menunggu) diatur oleh tim helpdesk.
                </p>
                {statusError && (
            ...
            <div className="px-6 py-4 border-t border-slate-100">
              <Button variant="outline" className="w-full rounded-xl" onClick={() => setSelectedTicket(null)}>
                Tutup
              </Button>
            </div>
          </div>
